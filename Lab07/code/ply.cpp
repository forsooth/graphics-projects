    ...
    */
    ...
    if (myfile.is_open()) { // if the file is accessable
        ...
        while (reading_header && getline(myfile, line)) {
            ...
            if (strcmp(token_pointer, "end_header") ==
                0) { reading_header = false; }
        }
...
void ply::render() {
    ...
    for (int i = 0; i < faceCount; i++) {
        ...
        setNormal(i, vertexList[index0].x, vertexList[index0].y,
                  vertexList[index0].z,
                  vertexList[index1].x, vertexList[index1].y,
                  vertexList[index1].z,
                  vertexList[index2].x, vertexList[index2].y,
                  vertexList[index2].z);
...
void ply::findEdges() {
    for (int i = 0; i < faceCount; ++i) {
        edge e1(faceList[i].vertexList[0], faceList[i].vertexList[1], i);
        edge e2(faceList[i].vertexList[1], faceList[i].vertexList[2], i);
        edge e3(faceList[i].vertexList[2], faceList[i].vertexList[0], i);
        bool addE1 = true;
        bool addE2 = true;
        bool addE3 = true;
        for (int j = 0; j < edgeCount; ++j) {
            if (edgeList[j].vertices[0] == e1.vertices[0] &&
                edgeList[j].vertices[1]) {
                addE1 = false;
                edgeList[j].faces[1] = i;
            }
            if (edgeList[j].vertices[0] == e2.vertices[0] &&
                    edgeList[j].vertices[1] == e2.vertices[1]) {
                addE2 = false;
                edgeList[j].faces[1] = i;
            }
            if (edgeList[j].vertices[0] == e3.vertices[0] &&
                    edgeList[j].vertices[1] == e3.vertices[1]) {
                addE3 = false;
                edgeList[j].faces[1] = i;
            }
        }
        ...
        if (addE1) {
            edgeList[edgeCount++] = e1;
        }

        if (addE2) {
            edgeList[edgeCount++] = e2;
        }

        if (addE3) {
            edgeList[edgeCount++] = e3;
        }
    }
}
...
void ply::renderSilhouette() {
    ...
    for (int i = 0; i < edgeCount; i++) {
        ...
        if (face1.dotProd * face2.dotProd < 0) {
...
void ply::setNormal(int facenum, float x1, float y1, float z1,
    ...
    faceList[facenum].dotProd = (cx * this->lookX) + cy + (cz * this->lookZ);
}
