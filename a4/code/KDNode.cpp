...
KDNode::KDNode(std::vector<const SceneOp *> sops, unsigned int depth) {
        ...
        for (const SceneOp *op : sops) {
                Box op_box = Box();
                op_box = op->mat * op_box; // TODO add support for mesh, for now op_box is standard
                if (after_first) {
        ...
        if (left == nullptr && right == nullptr) {
                ...
                bounding_box = Box(right->bounding_box.corner, right->bounding_box.dims);
        } else if (left != nullptr && right == nullptr) {
                bounding_box = Box(left->bounding_box.corner, left->bounding_box.dims);
        } else {
                bounding_box = Box(left->bounding_box.corner, left->bounding_box.dims);
...
void KDNode::intersected(std::vector<const SceneOp *> &intersect_obj, Point p, Vector ray) {
        if (!bounding_box.intersect_box(p, ray)) {
                return;
        }

        //intersect_obj.insert(std::end(intersect_obj), std::begin(sceneops), std::end(sceneops));
        //std::copy(sceneops.begin(), sceneops.end(), std::back_inserter(intersect_obj));
        for (const SceneOp *op : sceneops) {
                intersect_obj.emplace_back(op);
        }
