...
bool TiXmlDocument::LoadFile( FILE* file, TiXmlEncoding encoding )
{
    ...
    long int signed_length = ftell( file );
    if (signed_length < 0) {
        fprintf(stderr, "Could not get file stream position\n");
    } else {
        length = (unsigned long) signed_length;
    }
...
void TiXmlAttribute::SetDoubleValue( float _value )
{
    char buf [256];
#if defined(TIXML_SNPRINTF)
    TIXML_SNPRINTF(buf, sizeof(buf), "%f", (double)_value);
#else
