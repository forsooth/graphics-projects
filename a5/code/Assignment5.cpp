...
std::string filenamePath = "data/tests/all.xml";
...
int supersample_toggle;
int kdtree;
int shadows;
int reflections;
int floatalg;
int concurrent;
int worldspaceray = true;
int lens;
int precompute_mats;
...
SceneParser parser = SceneParser(filenamePath);
KDTree tree;
...
void color_pixel_block(Raycaster r, int i, const int num_threads) {
        ...
        for (int row = inital_row; row < last_row + 1; row++) {
                for (int col = 0; col < pixW + 1; col++) {
                        SceneColor c = r.raycast(col, row);
                        if (row < last_row) {
        ...
        for (int row = inital_row; row < last_row; row++) {
                for (int col = 0; col < pixW; col++) {
                        SceneColor c = r.raycast((real)col + 0.5f,
                                               (real)row + 0.5f);
...
void callback_start(int id) {
        ...
        Raycaster r = Raycaster(camera, flattener, parser, tree,
                                pixW, pixH, lens, depthlimit,
                                kdtree, shadows, reflections,
                                !worldspaceray, precompute_mats);
        ...
        double secdur = duration.count() / 1e6;
...
void callback_load(int id) {
        ...
        parser = SceneParser(filenamePath);
        ...
        if (!success) {
                ...
                tree = KDTree(flattener.flattened);
                set_up_camera();
        }
}
...
int main(int argc, char *argv[]) {
        ...
        GLUI_Panel *optpanel = glui->add_panel("Optimizations");
        glui->add_checkbox_to_panel(optpanel, "KD-Tree", &kdtree);
        glui->add_checkbox_to_panel(optpanel, "Concurrency", &concurrent);
        // glui->add_checkbox_to_panel(optpanel, "Float Algebra", &floatalg);
        glui->add_checkbox_to_panel(optpanel, "Precompute Matrices", &precompute_mats);
        // glui->add_checkbox_to_panel(optpanel, "World-Space Ray", &worldspaceray);
        (new GLUI_Spinner(optpanel, "Threads:", &num_threads))
                          ->set_int_limits(1, 32);

        glui->add_column_to_panel(optpanel, true);

        glui->add_checkbox_to_panel(optpanel, "Shadows", &shadows);
        glui->add_checkbox_to_panel(optpanel, "Reflections", &reflections);
        (new GLUI_Spinner(optpanel, "Recursion Depth:", &depthlimit))
                          ->set_int_limits(0, 15);


        glui->add_checkbox("Super Sample", &supersample_toggle);
