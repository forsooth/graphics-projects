...
void object::paintTexture(float x, float y, char r, char g, char b) {
    ...
    for (int xx = drawX - 5; xx < drawX + 5; xx++) {
        for (int yy = drawY - 5; yy < drawY + 5; yy++) {
            ...
            //    pixels.insert(Point(xx, yy, 0));
            }
        }
    }
    //for (auto pixel : pixels) {
    //  blendTexture->setPixel(pixel[0], pixel[1], r, g, b);
    //}
    ...
    */
    ...
    if (textureNumber <= 0) {
        if (baseTexture == nullptr) {
            ...
            baseTextureID = loadTexture(baseTexture->getWidth(),
        ...
        if (blendTexture == nullptr) {
            ...
            blendTextureID = loadTexture(blendTexture->getWidth(),
                                         blendTexture->getHeight(),
                                         blendTexture->getPixels());
        } else {
            delete blendTexture;
...
void object::drawTexturedSphere() {
    ...
    for (int i = 0; i < m_segmentsY; i++) {
        ...
        for (int j = 0; j < m_segmentsX; j++) {
            ...
            float newx_next = radius * cos(angleH + angleH_delta) *
                              cos(angle + angle_delta);
            float newz_next = radius * cos(angleH + angleH_delta) *
                              sin(angle + angle_delta);
            float newy_next = radius * sin(angleH + angleH_delta);

            // i = m_segmentsY = 100 maximum
            // j = m_segmentsY = 100 maximum

            float textureCoordX_delta =
                    (float) 1 / (float) m_segmentsX;    // increments with j
            float textureCoordY_delta =
                    (float) 1 / (float) m_segmentsY;    // increments with i
            ...
            // the texture.
            float tx =
                    1 - j * textureCoordX_delta;// starting x pixel coordinate
            float ty =
                    1 - i * textureCoordY_delta;// starting y pixel coordinate
            float etx = 1 - (j + 1) *
                            textureCoordX_delta;// ending x pixel coordinate
            float ety = 1 - (i - 1) *
                            textureCoordY_delta;// ending y pixel coordinate
